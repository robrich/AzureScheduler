    ...
    public class AzureRepository
    {
        ...
        public List<AzureInfo> GetForSubscription(Subscription Subscription)
        {
            ...
            using (ComputeManagementClient vmClient = new ComputeManagementClient(creds))
            {
                ...
                {
                    DeploymentGetResponse vmDetails = null;
                    try
                    {
                        vmDetails = vmClient.Deployments.GetByName(vm.ServiceName, vm.ServiceName);
                    }
                    catch (Exception)
                    {
                        vmDetails = null;
                    }
                    var azureInfo = new AzureInfo
                    {
                    ...
                    if (vmDetails != null)
                    {
                        azureInfo.Urls = new List<string> { vmDetails.Uri.OriginalString }.Union((from i in vmDetails.VirtualIPAddresses select i.Address)).ToList();
                        azureInfo.Status = vmDetails.Status.ToString();
                        azureInfo.Online = vmDetails.Status == DeploymentStatus.Running;
                    }
                    results.Add(azureInfo);
                }
            }
